            ...
            if (ret) {
                ...
                if (is->m_swrCtx) {
                    ...
                    if(is->m_playSpeed!=1.0) {
                        ...
                        int ret=sonicWriteShortToStream(is->m_sonicStream,(short*)is->m_audioBuf,len2);
                        ...
                        int bytes=numSamples*is->m_targetChannels*av_get_bytes_per_sample(is->m_targetSampleFmt);
                        if(bytes>out_size) {
                            av_fast_malloc(&is->m_audioBuf,&is->m_audioBufSize,bytes);
                        }
                        len2=sonicReadShortFromStream(is->m_sonicStream,(short*)is->m_audioBuf,numSamples);
                    }
                    is->m_audioBufSize = len2*is->m_targetChannels*av_get_bytes_per_sample(is->m_targetSampleFmt);
                }
                else {
...
double AVPlayer::computeTargetDelay(double delay)
{
    ...
    if(diff>1.0){
        qDebug()<<"VIDEOCLOCK:"<<m_videoClock.getClock()<<"AUDIOCLOCK: "<<m_audioClock.getClock()<<"VIDEO_AUDIO DIFF: "<<diff;
    }
    //计算同步阈值
...
{
    //再不跳转的情况下 需要判断上一帧和当前帧的差值当然这个基于是没有的，如果超出最坏阈值的话直接返回一帧的时间片，否则返回该值
    if(curFrame->serial==lastFrame->serial) {
        double duration=curFrame->pts-lastFrame->pts;
        //qDebug()<<"Frame: "<<duration;
